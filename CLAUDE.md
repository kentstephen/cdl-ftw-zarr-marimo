...
- THE BACKDROP WAS TRIED AND REVERTED (2026-08-26, a4f31fb reverted). `raster_dim`
  ...
  outlines + color by agreement): "this is so confusing", then "pls revert this
  feature". Why it failed, for the next person who reaches for it: the CDL palette
  and viridis COLLIDE IN HUE (CDL corn yellow next to viridis "agrees" yellow, CDL
  grapes purple next to viridis "disagrees" purple), so the surround argument does
  not hold; and in the Delta the between-field CDL is most of the frame, speckled,
  not a quiet ground. The raster and a field paint cannot share a frame. That was
  already his finding at raster_dim 0 ("perceptually baffling"); this is the second
  confirmation. STILL OPEN, then: the CDL raster switch stays checked-and-inert
  under a paint, which is what he was complaining about ("cdl raster is selected
  even when we don't see it"). The routes left are the two he did not pick: hide
  the switch under a paint (as crops only already does), or restructure the row
  into ONE exclusive selector (raster | fields: CDL | color by agreement |
  AlphaEarth suggests, click again for none) since these are modes, not layers.
