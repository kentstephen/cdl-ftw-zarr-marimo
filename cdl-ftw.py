...
def _():
    ...
    # (256-px tiles: a z13 image px is ~15 m at 38N, so 1.4 puts z13 on 20 m and z14
    # on native 10 m; 1.0 put z13 on native, 3x the rows for little on screen)
    ROW_BUDGET = 3_000_000        # analyze's level budget (the map is tiled now)
    TILE_PX = 256                 # the map is TILES (2026-08-20, late): deck asks for
    # z/x/y, the kernel answers PNGs. One SQL query per BATCH of requests (a
    # tile query costs ~0.2 s of fixed overhead on the xql table whatever its
    # size, measured; one query for a view's 24 tiles costs the same 0.2 s),
...
def _(
    ...
    def render_tiles(grid, gx0, gy1, pix_m, lut, boxes, px=256,
                     rings=None, ftw=None, clip=False, dis=False, noncrop=None,
                     dis_lut=None, sel=None, line=(40, 40, 40, 210)):
        """PNGs for a BATCH of tiles from ONE class-code grid (an xarray slice
        of the CDL level: rows north to south, columns west to east, `gx0` /
        `gy1` the Albers left / top EDGE, `pix_m` the pixel). Each tile's
        lon/lat lattice is forward-transformed into it (nearest) and takes
        its code. The FTW side is decided per output
        pixel from its lon/lat: `ftw` = (mask, ix0, iy0, res) is a dense
        boolean of the P(field) >= 0.5 grid; `clip` drops pixels outside a
        field cell; `dis` repaints by CDL crop/non-crop x FTW field (classes
        1/2/3 via `dis_lut`; non-crop and no field is not drawn). `sel`
        filters the paint AFTER the counts. Outline polylines (lon/lat) are
        drawn per tile with PIL. Returns (list of PNG bytes in `boxes`
        order, counts by code-or-class over drawn output pixels, drawn
        count, acres per output pixel)."""
        gh, gw = grid.shape
        x0, y1 = gx0, gy1
        keep = None
        if sel:
        ...
        for (W, S, E, N) in boxes:
            ...
            code = np.zeros((px, px), dtype=np.uint8)
            ...
            if ftw is not None:
                ...
                field = np.zeros((px, px), dtype=bool)
                field[inb] = mask[fy[inb], fx[inb]]
                if clip:
                    code[~field] = 0
            if dis:
                crop = (code > 0) & ~noncrop[code]
                cls = np.zeros((px, px), dtype=np.uint8)
                cls[crop & field] = 1
                cls[crop & ~field] = 2
            ...
            pxa = (((E - W) / px * 111.32 * math.cos(lat_mid))
                   * ((N - S) / px * 110.574)) * ACRES_PER_KM2
        return pngs, counts, ndrawn, pxa
...
def _(CartoStyle, HOLD: dict, HOME, Map, MaplibreBasemap):
    ...
    # shows the moment you move, new tiles fill in, there is no whole-picture
    # swap (the bitmap's judder) and no debounce/margin/held logic.
    HOLD.pop("k", None)
...
def _(
    ...
    def _drop_list():
        """Codes never drawn: 0 (background / fill), 81 (clouds), and the
        land-cover classes when crops-only is on."""
        return [0, 81] if not _crops_only else sorted({0, 81, *NONCROP_CODES})

    def _dsk(k):
        return (DS10 if _B == 10 else DS)[k]
    ...
    def _window(vs):
        """Level + Albers box for a view: floor pick, then the pixel budget."""
        k = _pick_level(vs)
        x0, y0, x1, y1 = albers_box(*bbox4326(vs))
        while k < _LV[-1] and (x1 - x0) * (y1 - y0) / (_B * k) ** 2 > ROW_BUDGET:
            k = _LV[_LV.index(k) + 1]
        return k, x0, y0, x1, y1, _drop_list()

    _LUT = np.zeros((256, 3), dtype=np.uint8)
    for _code, (_nm, _hx, _r, _g, _b, _nc) in CLASSES.items():
        _LUT[_code] = (_r, _g, _b)
    ...
    _META = {c: (v[0], v[1]) for c, v in CLASSES.items()}
    ...
    def _ftw_mask(px_m, W, S, E, N):
        ...
        if missing:
            ...
            # the P(field) window straight from the Zarr (xarray slice; rows
            # north to south), thresholded, dropped into the chunk-aligned box
            da = FTW_DS[f]["variables"].sel(time=f"{_fyear}-01-01", band="field").sel(
    ...
    # ---- THE TILE SERVE (2026-08-20, late). deck's TileLayer asks for
    # z/x/y tiles as the camera moves; `_fetch` answers from the cache or
    # joins a BATCH: the first request of a burst opens one, waits BATCH_S,
    # then ONE query runs for the union box of every tile that joined (a
    # per-tile query costs ~0.2 s of fixed overhead on the xql table whatever
    # its size, measured; one query for 24 tiles costs the same 0.2 s) and
    # the PNGs are cut per tile from the shared grid in a worker thread.
    # Followers await the batch's future (shielded: a cancelled tile request
    # must not abandon the batch). The FTW mask and outline tiles come from
    # their own caches over the same union box. -----------------------------
    _state = (_year, _fields, _dis, _crops_only, _sel, _B)
    _tiles = HOLD.setdefault("tiles", {})

    def _level_for(z, lat):
    ...
    def _serve_batch(z, keys):
        ...
        k = _level_for(z, (S + N) / 2)
        x0, y0, x1, y1 = albers_box(W, S, E, N)
        pix = float(_B * k)
        # the CDL window as a grid: one xarray slice of the level (no SQL,
        # no row expansion; the Zarr chunk read is the whole cost)
        da = _dsk(k).crop_type.sel(year=_year).sel(x=slice(x0, x1), y=slice(y1, y0))
        grid = np.array(da.values, dtype=np.uint8)
        npx = int(grid.size)
        if grid.size:
            grid[np.isin(grid, _drop_list())] = 0
        xs, ys = da.x.values, da.y.values
        gx0 = float(xs[0]) - pix / 2 if len(xs) else x0
        ...
        pngs, counts, ndrawn, pxa = render_tiles(
            grid, gx0, gy1, pix, _LUT, boxes, px=TILE_PX, rings=rings,
        ...
        line = (f"z{z} · {k}x · {_B * k} m pixels · {len(keys)} tiles · "
                f"{npx:,} px · {ndrawn:,} drawn · year {_year}"
                + (f" · FTW {_fyear} fields · {nt} outline tiles" if fields else "")
                + (f" · disagreement vs P(field) at {10 * f} m" if dis else "")
                + note + f" · {int((time.time() - t0) * 1000)} ms")
        HOLD["k"] = k
    ...
    def _on_vs(change):
        # the camera no longer serves anything (deck asks for tiles itself);
        # it is recorded for analyze and the SQL cells under the map
        try:
            vs = _vsd(change.new)
            if vs is None:
                return
    ...
    def _analyze_html(vs):
        ...
        # numpy version: the view as a lon/lat lattice (~one sample per CDL
        # pixel, capped), forward-transformed once; every count is a bincount
        # over the samples (the same per-pixel logic the tiles use), so the
        ...
        hm = (N - S) * 110_574
        nx = max(8, min(2000, int(wm / (_B * k))))
        ny = max(8, min(2000, int(hm / (_B * k))))
        lons = W + (np.arange(nx) + 0.5) * (E - W) / nx
        lats = N - (np.arange(ny) + 0.5) * (N - S) / ny
        LON, LAT = np.meshgrid(lons, lats)
        X, Y = albers_xy(LON, LAT)
        px_km2 = (wm / nx / 1000) * (hm / ny / 1000)   # one sample's ground area

        def _sample(ds, kk, year):
            da = ds.crop_type.sel(year=year).sel(x=slice(x0, x1), y=slice(y1, y0))
            g = np.asarray(da.values)
            if not g.size:
                return np.zeros(LON.shape, dtype=np.uint8)
            pix = float((10 if ds is DS10[kk] else 30) * kk)
            ...
            c = np.zeros(LON.shape, dtype=np.uint8)
            c[ok] = g[jy[ok], jx[ok]]
            return c

        field = None
        if fields or _dis:
            mask, fx0, fy0, res, _f = _ftw_mask(_B * k, W, S, E, N)
            fx = np.floor((LON + 180.0) / res).astype(np.int64) - fx0
            fy = np.floor((FTW_Y0 - LAT) / res).astype(np.int64) - fy0
            inb = (fx >= 0) & (fx < mask.shape[1]) & (fy >= 0) & (fy < mask.shape[0])
            field = np.zeros(LON.shape, dtype=bool)
            field[inb] = mask[fy[inb], fx[inb]]
        codes = _sample(_dsk(k), k, _year)
        codes[np.isin(codes, drop)] = 0
        if fields:
            codes[~field] = 0
        if _sel and not _dis:
            _keep = np.zeros(256, dtype=bool)
            _keep[list(_sel)] = True
            codes = np.where(_keep[codes], codes, 0)
        cnt = np.bincount(codes[codes > 0], minlength=256)
        order = [int(c) for c in np.argsort(-cnt) if cnt[c] > 0][:10]
        rows = [(_META[c][0], _META[c][1], int(cnt[c]), c) for c in order if c in _META]
        # the timelapse: 18 years of the 30 m group at the 30 m level nearest
        # this serve's pixel, the same lattice, the reads in a small pool
        _k30 = max(1, min(256, 2 ** round(math.log2(max(_B * k / 30, 1)))))
        _t_tl = time.time()
        _tl_codes = [r[3] for r in rows[:6]]

        def _year_counts(yr):
            c = _sample(DS[_k30], _k30, yr)
            if fields:
                c[~field] = 0
            return yr, np.bincount(c[c > 0], minlength=256)

        tl = []
        if _tl_codes:
            with ThreadPoolExecutor(max_workers=6) as _ex:
                for yr, c in _ex.map(_year_counts, YEARS):
                    tl.extend((yr, code, int(c[code])) for code in _tl_codes if c[code])
        ...
        if fields:
            extra = (f"FTW {_fyear} field area in view {field.sum() * px_km2 * ACRES_PER_KM2 / 1e3:,.1f}k acres"
        ...
        out.append(_timelapse_svg(rows[:6], tl, px_km2, _k30, tl_ms, fields))
        return "".join(out)
